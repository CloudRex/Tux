	...
	async executed(context) {
		...
		if (itemKey === "*" || itemKey === "all") {
			...
			if (items.length > 0) {
				...
				context.ok(`Sold ${items.length} item(s) for:small_orange_diamond:**${totalWorth}**. You now have:small_orange_diamond:**${points}**`);
			}
			else {
				context.fail(":sob: Oh noes! You don't have any items to sell.");
			}
		}
		else {
			...
			if (item) {
				...
				context.ok(`Sold :${item.key}:x1 for:small_orange_diamond:**${item.value}**. You now have:small_orange_diamond:**${points}**`);
			}
			else {
				context.fail("You don't own that item.");
			}
		}
	},
